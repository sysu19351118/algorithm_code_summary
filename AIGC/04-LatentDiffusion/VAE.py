...
class MNISTVAE(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx=None):
        ...
        recon_loss = F.binary_cross_entropy(x_recon, batch[0].reshape(batch[0].shape[0], -1), reduction='sum')

        # KL 散度损失 
        kl_loss = -0.5 * (1 + logvar - mu.pow(2) - logvar.exp()).sum(dim=1).sum() 
        total_loss = 10*recon_loss + kl_loss
        # 正确使用 self.log() 方法
